defmodule TpWeb.Views do
        ...
        Array.prototype.forEach.call(document.querySelectorAll('form[action="/messages/compose"]'), function ( form) {
          ...
          form.addEventListener('click', function ( event) {
            ...
            if ( button.classList.contains('js-current-time')) {
              ...
              if ( input) input.value = button.getAttribute('data-format') === 'hhmm' ? currentHhMm() : currentDdHhMm();
            }
          });
  ...
  defp compose_form("CHG", params), do: mini_flight_form("CHG", "Modification", params)
  defp compose_form("DLA", params), do: mini_flight_form("DLA", "Delay", params)
  ...
  defp ats_tooltip("item22"), do: "Amendment\n\nField Type 22 contains the amendment data for a CHG message. Insert the item number followed by the amended contents, for example: 13/WIII0400."
  ...
  defp change_form, do: mini_flight_form("CHG", "Change Message", %{})

  defp arrival_form(type \\ "ARR", title \\ "Arrival Message")
  defp arrival_form("ARR", _title), do: mini_arrival_form(%{})

  defp arrival_form(type, title) do
    ats_form(
      type,
      title,
      """
  ...
  defp mini_flight_form(type, title, params \\ %{}) do
    priority = compose_prefill(params, "priority", "FF")
    originator = compose_prefill(params, "originator", default_originator())
      ...
      <form id="#{String.downcase(type)}-form" class="aftn-form" method="post" action="/messages/compose">
        ...
        <div class="aftn-body">
          ...
          <div class="rcf-body mini-ats-body">
            <div class="alr-grid">
              <div class="alr-row mini-ats-row-head#{mini_time_class(type)}">
                #{ats_message_number(type)}
              ...
              <div class="alr-row mini-ats-row-dest">
                ...
                #{rcf_input("dof", "18. DOF/", "", required: true, maxlength: 6, prefix: "-", tooltip: alr_tooltip("dof"))}
              </div>
              #{mini_item22(type)}
              #{mini_filled_by()}
            </div>
  ...
  defp mini_arrival_form(params \\ %{}) do
    priority = compose_prefill(params, "priority", "FF")
    originator = compose_prefill(params, "originator", default_originator())

    """
    <section id="ARR" class="aftn-window">
      <div class="aftn-title"><i class="bi bi-airplane-engines"></i><span>ARR ( Arrival ) Message</span></div>
      <form id="arr-form" class="aftn-form" method="post" action="/messages/compose">
        <input type="hidden" name="compose_type" value="ARR">
        <input type="hidden" name="return_to" value="compose">
        <input type="hidden" name="return_form" value="ARR">
        #{ats_toolbar()}
        <input type="hidden" name="transmission_id" value="#{html(current_cid_seq())}" data-current-cid-seq>
        <div class="aftn-body">
          <div class="aftn-required-note">Blue field indicates required field.</div>
          ...
          <div class="rcf-body mini-ats-body">
            <div class="alr-grid">
              <div class="alr-row mini-arr-row">
                #{ats_message_number("ARR")}
                #{rcf_input("reference_data", "Reference Data", "", maxlength: 12, tooltip: alr_tooltip("reference_data"))}
                #{rcf_input("aircraft_id", "7. Aircraft ID", "", required: true, maxlength: 7, prefix: "-", tooltip: alr_tooltip("aircraft_id"))}
                #{rcf_check_field("ssr_mode", "SSR Mode", "A", prefix: "/", tooltip: alr_tooltip("ssr_mode"))}
                #{rcf_input("ssr_code", "Code", "", maxlength: 4, tooltip: alr_tooltip("ssr_code"))}
              ...
              <div class="alr-row mini-arr-row2">
                #{rcf_input("destination", "17. ARR AD", "", required: true, maxlength: 4, prefix: "-", tooltip: ats_tooltip("arrival_ad"))}
                #{rcf_time_input("arrival_time", "Time", "", required: true, maxlength: 4, tooltip: ats_tooltip("arrival_time"))}
                #{rcf_textarea("arrival_aerodrome", "Arrival Aerodrome", "", tooltip: ats_tooltip("arrival_aerodrome"))}
              </div>
              #{mini_filled_by()}
            </div>
          </div>
  ...
  defp ats_message_number(type) do
    ...
    <div class="field required rcf-message-type"#{message_type_tip}>
      ...
      <div class="alr-control compact">
        #{alr_mark("(")}
        <input name="message_type" value="#{html(type)}" readonly required maxlength="3"#{message_type_tip}>
  ...
  defp rcf_time_input(name, label, value, opts \\ []) do
    required = if Keyword.get(opts, :required, false), do: " required", else: ""
    maxlength = if max = Keyword.get(opts, :maxlength), do: ~s( maxlength="#{max}"), else: ""
    title = alr_title_attr(Keyword.get(opts, :tooltip) || alr_tooltip(name))
    input_id = "ats-#{name}"
    ...
    <div class="field#{required}"#{title}>
      <label#{title}>#{html(label)}</label>
      <div class="time-control">
        <input id="#{html(input_id)}" name="#{html(name)}" value="#{html(value)}"#{maxlength}#{required}#{title}>
        <button class="time-button js-current-time" type="button" data-target="#{html(input_id)}" data-format="hhmm" title="Use current HHMM" aria-label="Use current HHMM"><i class="bi bi-clock-history"></i></button>
      </div>
    </div>
    """
  end
