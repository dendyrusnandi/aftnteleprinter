...
public class TabOUT {
	...
	public static String sTblNm=/*"aftnrdI"*/"aftnrdO",sIdName="idcnt",sID=Titles.stOutgoing,sCriteria="";
...
//		Button bFrom = new Button(grp1, SWT.BORDER); sh.buttonStyle(bFrom, "", "", sh.widthNavBtn, sh.DarkGrey, SWT.CENTER, SWT.CENTER, Images.imgCalendar16); 
...
//		Button bTo = new Button(grp1, SWT.BORDER); sh.buttonStyle(bTo, "", "", sh.widthNavBtn, sh.DarkGrey, SWT.CENTER, SWT.CENTER, Images.imgCalendar16);
...
//		tFrom.setText("2015-08-04 02:11"); //buat nyobain aja
//		tTo.setText("2015-08-04 02:11");
...
//		new Lists().col(sID);
	...
	void searchData() {
		sTblNm=/*"aftnrdI"*/"aftnrdO"; 
		sIdName="idcnt"; 
		sID=Titles.stOutgoing;
		MainForm.clearTbl(); 
		new Lists().col(sID);
	...
	private void FindListener() {
		...
		if (!tTMS.getText().isEmpty()) critTMS=" AND firstdt LIKE '%"+tTMS.getText()+"%'";
		//karena dijadikan satu tabel, i.e. aftnrdIYYYY_MM, maka pencarian cid,seq,tms mengacu ke field outt
//		if (!tCID.getText().isEmpty()) critCID=tCID.getText();
//		if (!tCSNfr.getText().isEmpty()) critCSNfr=tCSNfr.getText();
//		if (!tCSNto.getText().isEmpty()) critCSNto=tCSNto.getText();
//		if (!tTMS.getText().isEmpty()) critTMS=tTMS.getText();
...
//		}
		...
		sCriteria=critCID+critCSNfr+critCSNto+critTMS+critFree;
		new Lists(sTblNm,sIdName,sID,sCriteria,pDate,pDateTo).Find();
		
//		sCriteria=/*critCID+critCSNfr+critCSNto+critTMS+*/critFree+" AND outt!=''";
//		new Lists(sTblNm,sIdName,sID,sCriteria,pDate,pDateTo,critCID,critCSNfr,critCSNto,critTMS).Find();
	}
}