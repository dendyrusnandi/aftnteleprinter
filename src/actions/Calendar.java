...
public class Calendar {
	...
	public Calendar(Shell shl) {
		shell = shl;
		formSetting();
		sh.shellStyle(shell, "Date/Time Properties");
	}
	...
	void formSetting() {
		...
		sh.labelStyle1(label, "Current time : ", SWT.RIGHT, SWT.CENTER, SWT.NORMAL, Colors.NORMAL);
		clTimer = new CLabel(gTime, SWT.NONE);
//		sh.clabelStyle(clTimer, "", 200, SWT.LEFT, SWT.CENTER, SWT.BOLD, Colors.NORMAL, Images.img_time);
		sh.clabel(clTimer, "00:00:00", SWT.LEFT, SWT.CENTER, Colors.NORMAL);
		label = new Label(gTime, SWT.NONE); 
		sh.labelStyle1(label, "Set new time : ", SWT.RIGHT, SWT.CENTER, SWT.NORMAL, Colors.NORMAL);
		Composite cNewTime = new Composite(gTime, SWT.NONE); sh.compositeStyle(cNewTime, 6, SWT.LEFT, SWT.CENTER);
		final Text tHour = new Text(cNewTime, SWT.BORDER); sh.textStyle(tHour, 25, 2, SWT.CENTER, SWT.CENTER, sh.numeric, "set Hour", true);
		label = new Label(cNewTime, SWT.NONE); sh.labelStyle1(label, ":", SWT.CENTER, SWT.CENTER, SWT.BOLD, Colors.NORMAL);
		...
		Composite comp = new Composite(dialog, SWT.NONE); sh.compositeStyle(comp, 3, SWT.CENTER, SWT.CENTER); 
		Button bOk = new Button(comp, SWT.PUSH); 
		Button bCancel = new Button(comp, SWT.PUSH); 
		
		sh.buttonStyle(bOk,"&OK","Set new date/time",90,Colors.DarkGrey,SWT.CENTER,SWT.CENTER,null);//Images.img_clear16);
		sh.buttonStyle(bCancel,"&Cancel","Cancel set new date/time",90,Colors.DarkGrey,SWT.CENTER,SWT.CENTER,null);//Images.img_clear16);
