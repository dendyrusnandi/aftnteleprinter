		...
		if (!shdisp.getDisplay().isDisposed()) {
			shdisp.getDisplay().syncExec(new Runnable() {
				public void run() {
					...
					shell.setText("Incoming list DEP, DLA, CNL AND CHG without FPL");
					label = new Label(shell, SWT.LEFT);
					label.setText("Incoming list DEP, DLA, CNL AND CHG without FPL");
					label.setBounds(10,10,335,20);
					...
					String sQuery = "SELECT * FROM incoming_msg WHERE stfpl='0' and (type='DEP' or type='CNL' or type='CHG' or type='DLA') order by tgl desc";
					if (jdbc.getExpiredDEP1(sQuery, table)) {
